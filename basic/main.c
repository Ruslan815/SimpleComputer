...
char* if_func(char* input_str)
{
    ...
    else if (sign == '<') { // не работает, если равны
...
{
    /*  char* tempExpressionString =*/ parseLetString(str, length);
    length = strlen(str);
    //    printf("%s\n", str);
    ...
    char answerVariable = str[0];
        ...
        //char expressionString[length - 1];
...
int main()
{
    ...
    if (!input) {
        ...
        while (getline(&input_str, &tmp_length, input) != -1) {
            ...
            //getline(&input_str, &tmp_length, input);
            //cout << input_str;
            //printf("%s \n", input_str);
