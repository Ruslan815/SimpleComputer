...
int Get_Command(char *str){
    if (!strcmp("READ",str)){
        ...
        return 0x20;
       }else if(!strcmp("STORE",str)){
        return 0x21;
       }else if(!strcmp("ADD",str)){
        return 0x30;
       }else if(!strcmp("SUB",str)){
        return 0x31;
       }else if(!strcmp("DIVIDE",str)){
        return 0x32;
       }else if(!strcmp("MUL",str)){
        return 0x33;
       }else if(!strcmp("JUMP",str)){
        return 0x40;
       }else if(!strcmp("JNEG",str)){
        return 0x41;
       }else if(!strcmp("JZ",str)){
        return 0x42;
       }else if(!strcmp("HALT",str)){
        return 0x43;
       }else if(!strcmp("JNP",str)){
        return 0x59;
       }else if(!strcmp("=",str)){
        return 1;
       }else {
...
int Read_String(char *str, int length){
    int i, address, command, operand = 0;
    char str_c[7];
    FILE *output = fopen ("SA.dat", "wb");
    address = (str[0] - 0)*10 + (str[1] - 0);
    i = 3;
    while (str[i] != ' '){
        str_c[i-3] = str[i];
        i++;
    ...
    for (int j = i; (j=str[j] >= '0' && str[j] <= '9') || str[j] == '+'; j ++){
        if (str[j] != '+' ){
